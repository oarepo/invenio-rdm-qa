...
// VERIFICATION ------------------------------------------------------------------------
  ...
  try {
      ...
      if (error instanceof Error) {
          ...
          console.error(`Restricted status label not found within ${timeout} ms: ${String(error)}`);
      }
      return false;
  }
}
...
async checkRestrictedMessagePresence(timeout: number = 10000): Promise<boolean> {
  ...
  try {
      ...
      if (error instanceof Error) {
          ...
          console.error(`Restricted message not found within ${timeout} ms: ${String(error)}`);
      }
      return false;
  }
}
