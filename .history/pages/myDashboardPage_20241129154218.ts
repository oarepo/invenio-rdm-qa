...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async getCommunityLabel(labelText: string): Promise<string> {
    // Use a more specific locator with the text content
    const labelElement = this.page.locator(`a.ui.medium.header.mb-0:has-text("${labelText}")`);
  ...
  async compareCommunityLabelWithInvitation(index: number, labelText: string): Promise<boolean> {
    const communityLabel = await this.getCommunityLabel(labelText); // Pass the expected label text
    const invitationText = await this.saveInvitationTextInQuotes(index);
