...
// BUTTONS -----------------------------------------------------------------------------
  ...
  // Method to click the 'Accept and publish' button with visibility check
  async clickAcceptAndPublishButtonWait() {
    const buttonLocator = this.page.locator('button:has-text("Accept and publish")');
  
    // Ensure the button is visible before attempting to click
    await buttonLocator.nth(1).waitFor({ state: 'visible' });
  
    // Now, click the second button (index 1)
