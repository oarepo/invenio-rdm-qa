...
// FIELDS ------------------------------------------------------------------------------
  ...
  async fillUser(): Promise<void> {
    ...
    const comboboxLocator = this.page.locator('div.field:has(label:has-text("User")) div.ui.active.visible.multiple.search.selection.dropdown i.search.icon');
  ...
  async interactWithCombobox(): Promise<void> {
    ...
    const userEmail = testData.existingUserEmail;
    const comboboxContainer = this.page.locator('.ui.active.visible.multiple.search.selection.dropdown');
    const inputLocator = comboboxContainer.locator('input.search');
    const searchIconLocator = comboboxContainer.locator('i.search.icon');
    await comboboxContainer.fill(userEmail);
