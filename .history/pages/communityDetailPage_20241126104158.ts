...
// NAVIGATION --------------------------------------------------------------------------
  ...
  async navigateToFirstCommunity() {
    ...
    await this.page.waitForSelector('div.centered.image.fallback_image', { state: 'visible' });
...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async verifyRestrictedLabel(): Promise<boolean> {
    const restrictedLabelLocator = this.page.locator('i.ban.icon >> visible=true');
    const restrictedTextLocator = this.page.locator('text=Restricted');
    
    // Check if both the icon and the text "Restricted" are present
    const isIconVisible = await restrictedLabelLocator.isVisible();
    const isTextVisible = await restrictedTextLocator.isVisible();
    
    return isIconVisible && isTextVisible;
  }
}