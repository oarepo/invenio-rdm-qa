...
/**
 * This class provides a structured abstraction for interacting with the My Dashboard UI.
 * It encapsulates navigation, actions on user records and community requests, and result verification.
 *
 * ─────────────────────────────────────────────────────────────
 * INCLUDES:
 * ─────────────────────────────────────────────────────────────
 * 1. Navigation:
 *    - Navigate to My Dashboard, Communities, Requests, and Record Detail pages.
 *
 * 2. Interaction:
 *    - Click buttons such as Edit, Accept, Decline, and toggles like 'View all versions'.
 *    - Confirm community invitations and record requests.
 *
 * 3. Verification:
 *    - Check if a record was successfully deleted.
 *    - Confirm presence of labels (e.g., 'New version draft', 'All done!').
 *    - Match record titles to ensure saved data appears correctly.
 *
 * ─────────────────────────────────────────────────────────────
 * EXAMPLE USAGE:
 * ─────────────────────────────────────────────────────────────
 * const dashboard = new MyDashboard(page);
 * await dashboard.navigateToMyDashboard();
 * await dashboard.clickEditButton();
 * const isAbsent = await dashboard.verifyRecordIsAbsent('Test Record');
 */
