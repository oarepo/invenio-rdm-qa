...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async selectFileFormat(style: string): Promise<void> {
    // Click the dropdown to open the options (forcing the click to ensure interaction)
    const dropdown = this.page.locator('.ui.selection.dropdown.export-dropdown');
    await dropdown.click({ force: true });
  
    // Verify the dropdown menu is visible
    const menu = this.page.locator('.ui.selection.dropdown.export-dropdown .menu');
    await menu.waitFor({ state: 'visible' });
  
    // Locate the dropdown option by its text and click it
    const option = menu.locator(`.item:has-text("${style}")`);
    if (!(await option.isVisible())) {
      throw new Error(`Dropdown option "${style}" is not visible.`);
    }
    await option.click();
  
    // Verify that the selected style is displayed in the dropdown
    const selected = this.page.locator('.ui.selection.dropdown.export-dropdown .divider.text');
    await selected.waitFor({ state: 'attached' });
    const selectedText = await selected.textContent();
    if (selectedText?.trim() !== style) {
      throw new Error(`Failed to select the correct file format. Expected: "${style}", but got: "${selectedText?.trim()}".`);
    }
  }
