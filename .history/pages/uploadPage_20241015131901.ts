...
export class UploadPage {
  ...
  async selectResourceType() {
    const randomResourceType = testData.upload.resourceType();
...
// Use a more flexible selector for the iframe (ID starting with 'tiny-react')
  const iframeElement = this.page.frameLocator('iframe[id^="tiny-react"]'); 

  // Select the first iframe (if there are multiple) and then locate the body element inside the iframe
  const body = iframeElement.first().locator('body#tinymce');

  // Fill the body content with the provided description
