...
// FIELDS ------------------------------------------------------------------------------  
  ...
  async fillMember() {
    ...
    const searchIconSelector = 'div.field:has-text("Member") div.ui.fluid.multiple.search.selection.dropdown i.search.icon';
    await this.page.click(searchIconSelector);
    await this.page.locator('div.ui.active.visible.fluid.multiple.search.selection.dropdown input.search').fill(userEmail);
  }
