...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickSelectCasteButton() {
    // Find the div that contains the community name (link or text) and select the button in the same parent element
    const communityName = 'excepturi';  // Dynamically set this name or pass as a parameter
    await this.page.locator(`a[aria-label="${communityName}"] >> ancestor::div//button[aria-label="Select ${communityName}"]`).click();
  }  
