...
// FIELDS ------------------------------------------------------------------------------
  ...
  async selectResourceType(currentlySelected: string | null) {
    ...
    const resourceTypeField = this.page.locator('#rdm-deposit-form .required.field.invenio-select-field > div');
    await resourceTypeField.waitFor({ state: 'visible' });
    await resourceTypeField.click();
  
    // Wait for the option to be visible before clicking
    const optionLocator = this.page.getByRole('option', { name: randomResourceType, exact: true });
  ...
  async changeResourceType(currentlySelected: string | null) {
    ...
    const optionLocator = this.page.getByRole('option', { name: changeResourceType, exact: true });
    await optionLocator.waitFor({ state: 'visible' });
    await optionLocator.click();
  }
...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickSelectButton(communityName: string) {
    await this.page.getByRole('button', { name: `Select ${communityName}` }).click();
  }
