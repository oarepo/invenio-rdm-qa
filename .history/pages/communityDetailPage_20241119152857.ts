...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to verify the "results found" element contains a label with the correct number
  async verifyResultsFoundWithLabel(expectedLabel: string, expectedResultsText: string) {
    const resultsLocator = this.page.locator('div.left.aligned.four.wide.column');
    const labelLocator = resultsLocator.locator('div.ui.label');

    // Wait for the results element to be visible
    await expect(resultsLocator).toBeVisible();

    // Verify the label's text is the expected number
    const actualLabelText = await labelLocator.textContent();
    expect(actualLabelText?.trim()).toBe(expectedLabel);

    // Verify the parent element's text contains the expected results text
    const actualResultsText = await resultsLocator.textContent();
    expect(actualResultsText).not.toBeNull();
    await expect(resultsLocator).toHaveText(expectedResultsText);

    console.log(
      `Verified the "results found" element with label "${expectedLabel}" and text: "${expectedResultsText}".`
    );
  }
}