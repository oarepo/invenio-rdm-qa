...
// Method to select 'Visibility' from the dropdown menu 
  ...
  async selectVisibilityOption(role: string): Promise<void> {
    const dropdown = this.page.locator('//div[@aria-label="Visibility Hidden" or @aria-label="Visibility Public"]');
  
    // Click the dropdown to expand the menu
    await dropdown.click();
  
    // Wait for the dropdown menu to be visible
    const dropdownMenu = this.page.locator('//div[contains(@class, "menu transition") and contains(@class, "visible")]');
    await dropdownMenu.waitFor({ state: 'visible' });
  
    // Locate the desired option using the 'role' parameter
    const dropdownOption = dropdownMenu.locator(`//div[@role="option"]//strong[text()="${role}"]`);
