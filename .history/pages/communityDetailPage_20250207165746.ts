...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickEmbargoedCheckbox(accessStatus: string) {
    const checkboxLocator = `(//input[@aria-label="${accessStatus}"]/ancestor::div[@class="ui checkbox"])[2]`;
...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async isNumberPresent(expectedNumber: number): Promise<boolean> {
    ...
    // Wait for the element to be visible (timeout: 5 seconds)
    await labelLocator.waitFor({ state: 'visible', timeout: 5000 });

    // Get text content
    const textContent = await labelLocator.textContent();

    console.log(`Label text content: '${textContent?.trim()}'`); // Debugging output

    // Check if the text matches the expected number
    return textContent?.trim() === expectedNumber.toString();
  }
