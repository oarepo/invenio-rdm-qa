...
// FIELDS ------------------------------------------------------------------------------
 ...
 async fillUser(): Promise<void> {
   ...
   const memberFieldLocator = 'div[role="combobox"][class~="fluid"]';
...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickAddPeopleButton(): Promise<void> {
    const addPeopleButton = this.page.locator('button.ui.medium.icon.positive.right.floated.left.labeled.button.mobile.only.tablet.only.mb-15:has-text("Add people")');
    await addPeopleButton.waitFor({ state: 'visible' });
