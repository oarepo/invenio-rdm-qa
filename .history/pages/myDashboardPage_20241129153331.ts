...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async getCommunityLabel(): Promise<string> {
    const labelElement = this.page.locator('a.ui.medium.header.mb-0');
    ...
    return labelText; // TypeScript will now infer this as a string
  }
  ...
  async compareCommunityLabelWithInvitation(index: number): Promise<boolean> {
    ...
    const invitationText = await this.saveInvitationTextInQuotes(index); // Get the invitation text from the specified index

    if (communityLabel && invitationText) {
      const isEqual = communityLabel === invitationText; // Compare the two values
