...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to verify that the 'All done!' header is present on the page
  async verifyAllDoneHeader() {
    const headerLocator = this.page.locator('h1.ui.icon.header:has-text("All done!")');
  
    // Wait for the header to be visible
    await headerLocator.waitFor({ state: 'visible' });
  
    // Optionally, you can assert its presence
    const isHeaderVisible = await headerLocator.isVisible();
    expect(isHeaderVisible).toBe(true); // Ensure the header is visible
  }
}