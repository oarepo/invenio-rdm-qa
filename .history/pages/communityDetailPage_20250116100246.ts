...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async verifySelectedRole(expectedRole: string): Promise<void> {
    const selectedRole = this.page.locator('//div[@aria-label="Role owner"]//strong');
    await selectedRole.waitFor({ state: 'visible' });
    const actualRole = await selectedRole.textContent();
