...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickFilesOnlyRestrictedButton(timeout: number = 10000) {
    const buttonSelector = '(//button[@data-testid="protection-buttons-component-restricted"])[2]';

    try {
        // Wait for the button to become visible within the provided timeout
        await this.page.waitForSelector(buttonSelector, { state: 'visible', timeout });

        // Confirm the button is enabled before attempting to click
        const button = this.page.locator(buttonSelector);
        const isEnabled = await button.isEnabled();
        if (!isEnabled) {
            throw new Error(`The 'Files only -> Restricted' button is not enabled.`);
        }

        // Click the button
        await button.click();
        console.log('Clicked the "Files only -> Restricted" button successfully.');
    } catch (error) {
        console.error(`Failed to click the 'Files only -> Restricted' button: ${error.message}`);
