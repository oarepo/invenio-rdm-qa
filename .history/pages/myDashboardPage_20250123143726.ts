...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to check if any <a> element text matches the given title
  async isRecordTitleMatching(title: string): Promise<boolean> {
    // Locate all <a> elements
    const recordLinks = this.page.locator('a.truncate-lines-2'); // Update selector if needed

    // Retrieve the count of <a> elements
    const count = await recordLinks.count();

    for (let i = 0; i < count; i++) {
      // Retrieve the text content of the <a> element at index i
