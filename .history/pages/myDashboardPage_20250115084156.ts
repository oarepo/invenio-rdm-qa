...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickAcceptButton(index: number): Promise<void> {
    // Locate the 'Accept' buttons using XPath
    const acceptButtons = this.page.locator('(//button[contains(text(), "Accept")])');
    await acceptButtons.nth(index).waitFor({ state: 'visible' });
