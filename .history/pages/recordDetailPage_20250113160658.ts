...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to verify the 'Share' button is not present on the page anymore
  async verifyEditButtonNotPresent(): Promise<boolean> {
  const shareButton = this.page.locator('//div[@id="recordManagement"]//button[contains(@class, "ui") and contains(text(), "Share")]');
  const buttonCount = await shareButton.count();
