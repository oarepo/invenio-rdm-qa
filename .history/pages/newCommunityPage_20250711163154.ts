...
/**
 * This class provides a structured abstraction for creating a new community within the UI.
 * It includes all necessary navigation steps, field inputs, button interactions, and validation
 * logic required for successful creation and verification of a new community.
 *
 * ─────────────────────────────────────────────────────────────
 * INCLUDES:
 * ─────────────────────────────────────────────────────────────
 * 1. Navigation:
 *    - Navigate to Home, Communities, My Dashboard, New Community form, and Record Detail pages.
 *
 * 2. Interaction:
 *    - Fill in the 'Community name' and 'Identifier' fields (using test data + randomness).
 *    - Select visibility (Public / Restricted) and create the community via UI.
 *
 * 3. Verification:
 *    - Validate that the created community name appears correctly on the detail page.
 *    - Support retrieval of community name for further use.
 *
 * ─────────────────────────────────────────────────────────────
 * EXAMPLE USAGE:
 * ─────────────────────────────────────────────────────────────
 ...
 * const name = await newCommunity.fillCommunityName();
 * await newCommunity.fillCommunityIdentifier();
 * await newCommunity.selectPublicCommunity();
 * await newCommunity.clickCreateCommunity();
 * await newCommunity.verifyCommunityName(name);
 */
