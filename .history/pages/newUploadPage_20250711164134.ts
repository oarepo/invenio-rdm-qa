...
/**
 * This class provides a comprehensive abstraction for interacting with the upload page UI.
 * It covers navigation, field input, file uploading, button actions, and verification steps
 * needed to create, edit, publish, and manage uploaded records.
 *
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * INCLUDES:
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * 1. Navigation:
 *    - Navigate to Home, Upload section, My Dashboard, and record detail pages.
 *
 * 2. Interaction:
 *    - Upload files using a helper.
 *    - Fill various metadata fields (title, family name, description, publication date, embargo details).
 *    - Select options like DOI, resource type, access rights, visibility.
 *    - Interact with dialogs and modals for adding users, selecting communities, submitting for review, publishing.
 *    - Manage embargo settings and metadata-only options.
 *
 * 3. Buttons:
 *    - Click buttons for Save, Publish, Edit, Save draft, Preview, Share, Add People, Select community, Submit for review,
 *      Accept and publish, Discard changes, Delete, and others related to record management.
 *
 * 4. Verification:
 *    - Verify success messages, draft save messages, validation errors, record existence, metadata labels, user share additions,
 *      status confirmations, and UI element presence.
 *
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * EXAMPLE USAGE:
 * ─────────────────────────────────────────────────────────────────────────────────────────
 ...
 * await uploadPage.fillTitle('My Test Record');
 * await uploadPage.fillFamilyName('Smith');
 * await uploadPage.selectDOIOption(true);
 * await uploadPage.selectResourceType(null);
 * await uploadPage.fillPublicationDateWithToday();
 * await uploadPage.fillDescription('Description text here');
 * await uploadPage.clickSave();
 * await uploadPage.verifySuccessMessage('Record created successfully');
 */
...
export class UploadPage {
  ...
  async checkFirstMetadataOnlyCheckbox() {
    const checkboxLocator = this.page.locator("label", {
      hasText: "Metadata-only record",
    });
  ...
  async fillFamilyName(familyName: string) {
    ...
    const addCreatorButton = this.page.getByRole("button", {
      name: "Add creator",
    });
    await addCreatorButton.click();

    // Wait for the pop-up dialog to appear
    const familyNameInputLocator = this.page.waitForSelector(
    ...
    const saveButton = this.page.locator(
      "body > div.ui.top.aligned.page.modals.dimmer.transition.visible.active > div > div.actions > button:nth-child(3)"
    );
  ...
  async selectFirstResourceType() {
    const firstElement = this.page
      .locator("#rdm-deposit-form .required.field.invenio-select-field > div")
      .first();
  ...
  async selectResourceType(currentlySelected: string | null) {
    ...
    await this.page.click(
      "#rdm-deposit-form .required.field.invenio-select-field > div"
    );
    await this.page.waitForTimeout(500);
    await this.page
      .getByRole("option", { name: randomResourceType, exact: true })
      .click();
  }
  ...
  async changeResourceType(currentlySelected: string | null) {
    ...
    await this.page
      .getByRole("option", { name: changeResourceType, exact: true })
      .click();
  }
  ...
  async fillPublicationDateWithToday() {
    ...
    const publicationDateInput = this.page.locator(
      'input[name="metadata.publication_date"]'
    );
  ...
  async setEmbargoUntilDate() {
    ...
    const embargoDateInput = this.page.locator(
      'input[name="access.embargo.until"]'
    );
  ...
  async fillEmbargoReason() {
    ...
    const embargoReasonTextarea = this.page.locator(
      'textarea[name="access.embargo.reason"]'
    );
  ...
  async fillUser(): Promise<void> {
    ...
    const comboboxLocator = this.page.locator(
      '//label[text()="User"]//ancestor::form[@class="ui form"]//div[@role="combobox"]//input[@type="text"]'
    );
    await comboboxLocator.fill(userEmail);
    const selectOption = this.page.locator(
      '//label[text()="User"]//ancestor::form[@class="ui form"]//div[@role="combobox"]//div[@role="option"]/div'
    );
  ...
  async clickPublish() {
    await this.page.waitForSelector('button:has-text("Publish")', {
      state: "visible",
    });
  ...
  async clickShare() {
    const button = await this.page.locator(
      '//*[@id="rdm-deposit-form"]//button[contains(@class, "ui") and contains(@class, "icon") and contains(text(), "Share")]'
    );
  ...
  async clickAddPeopleButton(): Promise<void> {
    const addPeopleButton = this.page.locator(
      'th.center.aligned.three.wide button:has-text("Add people")'
    );
  ...
  try {
    ...
    if (error instanceof Error) {
      console.error(
        `Failed to click the "Select ${communityName}" button: ${error.message}`
      );
    } else {
      console.error(
        `Failed to click the "Select ${communityName}" button: ${String(error)}`
      );
    }
    throw error;
  }
  ...
  async clickAcceptAndPublishButton() {
    const button = this.page
      .locator('button:has-text("Accept and publish")')
      .first();
  ...
  async clickConfirmationAcceptAndPublishButton() {
    const confirmationButton = this.page
      .locator('button:has-text("Accept and publish")')
      .nth(1);
    await confirmationButton.waitFor({ state: "visible", timeout: 5000 });
  ...
  async clickFullRecordRestrictedButton() {
    await this.page.waitForSelector(
      '(//button[@data-testid="protection-buttons-component-restricted"])[1]'
    );
    await this.page.click(
      '(//button[@data-testid="protection-buttons-component-restricted"])[1]'
    );
  ...
  async clickFilesOnlyRestrictedButton() {
    ...
    await this.page.click(
      '(//button[@data-testid="protection-buttons-component-restricted"])[2]'
    );
  }
  ...
  async clickDiscardChanges() {
    await this.page.waitForSelector(
      "//div[@id='rdm-deposit-form']//button[contains(text(), 'Discard changes')]"
    );
    await this.page.click(
      "//div[@id='rdm-deposit-form']//button[contains(text(), 'Discard changes')]"
    );
  ...
  async checkEmbargoCheckbox() {
    const checkbox = await this.page.locator(
      'label.header[for="access.embargo.active"]'
    );
  ...
  async clickDeleteButton() {
    await this.page.waitForSelector(
      '//button[contains(@class, "ui fluid icon left labeled button negative") and text()="Delete"]'
    );
    await this.page
      .locator(
        '//button[contains(@class, "ui fluid icon left labeled button negative") and text()="Delete"]'
  ...
  async clickDeleteButtonConfirm() {
    await this.page.waitForSelector(
      '//button[contains(@class, "ui button negative")][.//text()="Delete"]'
    );
    await this.page
      .locator(
        '//button[contains(@class, "ui button negative")][.//text()="Delete"]'
  ...
  // Method to click the 'Add people' header
  async clickAddPeopleHeader(): Promise<void> {
    const headerLocator = this.page.locator('h2.header:text("Add people")');
    await headerLocator.click();
  }

  // Select 'Access' radio button (1 = 'Can view', 2 = 'Can preview drafts', 3 = 'Can edit', 4 = 'Can manage')
  async clickCanEditRadioButton(index: number): Promise<void> {
    const radioButtonLocator = this.page
      .locator(
        '(//label[text()="Access"]//ancestor::div[@class="required field"]//div[contains(@class,"radio checkbox")])'
      )
      .nth(index - 1);
    await radioButtonLocator.click();
  }

  // Method to click the 'Add' button ('Add people' pop-up dialog)
  async clickAddButton(): Promise<void> {
    const addButtonLocator = this.page.locator(
      "button.ui.icon.primary.left.labeled.button",
      { hasText: "Add" }
    );
  ...
  async verifySaveDraftMessage() {
    const successMessage = this.page.locator(
      "div.left.aligned.fifteen.wide.column >> strong"
    );
  ...
  async checkRecordExists() {
    ...
    await this.page.waitForSelector("#record-title", {
      state: "visible",
      timeout: 10000,
    });
  ...
  async waitForUploadedFilesTable() {
    const firstRow = this.page
      .locator("table.ui.striped.table.files.fluid.open tbody tr")
      .first();
  ...
  async verifyValidationErrors() {
    const resourceTypeError = this.page.locator(
      "div.left.aligned.fifteen.wide.column ul.list li.content",
      { hasText: "Resource type" }
    );
    const creatorsError = this.page.locator(
      "div.left.aligned.fifteen.wide.column ul.list li.content",
      { hasText: "Creators" }
    );
    const titleError = this.page.locator(
      "div.left.aligned.fifteen.wide.column ul.list li.content",
      { hasText: "Title" }
    );
    const doiError = this.page.locator(
      "div.left.aligned.fifteen.wide.column ul.list li.content",
      { hasText: "DOI" }
    );
  ...
  async verifyMetadataOnlyLabel() {
    const metadataLabel = this.page.locator(
      'span.metadata-only >> span:has-text("Metadata-only")'
    );
  ...
  async verifySubmittedStatusPresence() {
    const submittedElement = this.page.locator(
      'i.clock.outline.icon + span:has-text("Submitted")'
    );
  ...
  async verifyConfirmationStatusPresence() {
    const confirmationElement = this.page.locator(
      'i.check.circle.icon + span:has-text("Accepted")'
    );
  ...
  async verifyAddedUserForShare(): Promise<boolean> {
    const elementLocator = this.page.locator(
      '//div[contains(@class, "ui image label")]//img[@alt=""]'
    );
    const isPresent = (await elementLocator.count()) > 0;
  ...
  // Method to check if the 'Can edit' element exists on the page
  async isCanEditElementPresent(): Promise<boolean> {
    const elementLocator = this.page.locator(
      'div[role="alert"][class="divider text"]:has-text("Can edit")'
    );
    await this.page.waitForTimeout(1000);
