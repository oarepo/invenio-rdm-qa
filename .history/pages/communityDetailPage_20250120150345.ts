...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to verify if the 'Members' is visible in the list of members ('Members' section)
  async verifyMembersHeaderVisible(): Promise<boolean> {
    const membersHeaderLocator = this.page.locator('th:has-text("Members")');
    const isVisible = await membersHeaderLocator.isVisible();
    return isVisible;
  }
