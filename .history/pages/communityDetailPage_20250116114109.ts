...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to verify if the role was successfully set to Manager
async verifyRoleIsManager(index: number = 0): Promise<void> {
  // Locate all elements matching the role
  const roleManagerLocator = this.page.locator('//div[@aria-label="Role manager"]/div[text()="Manager"]');

  // Select the element by index
  const specificRoleLocator = roleManagerLocator.nth(index);

  // Wait for the specific locator to be visible
  await specificRoleLocator.waitFor({ state: 'visible' });

  // Assert that the specific locator is visible
  const isVisible = await specificRoleLocator.isVisible();
  if (!isVisible) {
    throw new Error(`The role was not set to "Manager" successfully for index ${index}.`);
  }
}
