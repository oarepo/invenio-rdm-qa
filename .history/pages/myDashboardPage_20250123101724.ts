...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async isNewVersionDraftLabelPresent(): Promise<boolean> {
    try {
      await this.page.waitForSelector('text="New version draft"', { state: 'visible', timeout: 5000 });
   ...
   // Method to check if the <a> element text matches the given title
   async isRecordTitleMatching(title: string): Promise<boolean> {
    // Locate the <a> element
    const recordLink = this.page.locator('a.truncate-lines-2'); // Update selector if needed

    // Retrieve the text content of the <a> element
    const recordText = await recordLink.textContent();

    // Check if the text is defined and matches the given title
    if (recordText) {
      const isMatch = recordText.trim() === title;
      console.log(`Comparing "${title}" with "${recordText.trim()}": ${isMatch}`);
      return isMatch;
    } else {
      console.log('Record text is empty or not found.');
      return false;
    }
  }
