...
// FIELDS ------------------------------------------------------------------------------  
  ...
  async fillMember(): Promise<void> {
    ...
    // Locator for the search icon and input within the "Members" section
    ...
    // Wait for the input field to be visible and interactive
    await this.page.waitForSelector(`${memberFieldLocator} input.search`, { state: 'visible' });

    // Fill in the user email in the input field
    await this.page.locator(`${memberFieldLocator} input.search`).fill(userEmail);
  }
