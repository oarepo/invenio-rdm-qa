...
// FIELDS ------------------------------------------------------------------------------
  ...
  async fillUser(): Promise<void> {
    ...
    // Refine the locator to specifically target the 'Add people' combobox
    const memberFieldLocator = 'div[role="combobox"][aria-label="Add people"]';
    const searchIconLocator = `${memberFieldLocator} i.search.icon`;
    const inputLocator = `${memberFieldLocator} input.search`;

    // Click the search icon within the 'Member' field to open the dropdown
    await this.page.click(searchIconLocator);
    await this.page.waitForSelector(inputLocator, { state: 'visible' });

    // Fill in the user email in the input field
    const inputElement = this.page.locator(inputLocator);
    await inputElement.fill(userEmail);
