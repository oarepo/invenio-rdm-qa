...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async selectButtonByText(communityName:string) {
     // Locate the <a> element with the community name as text
  const communityAnchor = this.page.locator(`a:has-text("${communityName}")`);

  // Find the parent container that holds both the anchor and the button
  const communityItem = communityAnchor.locator('>> xpath=ancestor::div[contains(@class, "community-item")]');

  // Now, locate the "Select" button within that parent and click it
  await communityItem.locator('button.ui.tiny.button').click();
}
