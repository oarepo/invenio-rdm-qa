...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to verify if the "Metadata-only" label is present (with a specific index)
  async verifyMetadataOnlyLabel(index: number) {
    const metadataLabel = this.page.locator(`div.ui.checkbox >> span:has-text("Metadata-only") >> nth-of-type(${index})`);
