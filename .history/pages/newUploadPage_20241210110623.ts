...
async fillUser(): Promise<void> {
  ...
  // Locate the parent element uniquely, then target the search icon within it
  const comboboxLocator = this.page.locator(
    'div.field:has(label:has-text("User")) div.ui.active.visible.multiple.search.selection.dropdown i.search.icon'
  );

  await comboboxLocator.click(); // Click the search icon
  const inputLocator = this.page.locator(
    'div.field:has(label:has-text("User")) div.ui.active.visible.multiple.search.selection.dropdown input.search'
  );

  await inputLocator.fill(userEmail); // Fill the user email in the input field
  await inputLocator.press('Enter'); // Press 'Enter' to confirm the input
}
