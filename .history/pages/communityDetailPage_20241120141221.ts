...
// NAVIGATION --------------------------------------------------------------------------
  ...
  async navigateToCommunities() {
    ...
    await this.page.waitForSelector('h1.ui.header:has-text("Communities")', { state: 'visible' });
  }

  // Navigate to the 'Communities' page using the helper
  async navigateToNewCommunities() {
    await this.uiHelper.goto(urls.newCommunityURL);
    await this.page.waitForSelector('h1.ui.header.rel-mt-2:has-text("Setup your new community")', { state: 'visible' });
  }
