...
export class UploadPage {
  ...
  async navigateToUploadSection() {
    ...
    await this.page.waitForURL(urls.newUploadURL);
     // Ensure that a key element (e.g., the upload form) is visible and ready
  await this.page.waitForSelector('form#upload-form', { state: 'visible', timeout: 10000 }); // Adjust the selector based on the actual form in the upload page

  // Confirm you're on the correct page by checking the URL
  expect(this.page.url()).toContain('https://inveniordm.web.cern.ch/uploads/new'); // Adjust as needed

  // Optional: Log to confirm navigation was successful
  console.log('Successfully navigated to the upload section.');
  }
