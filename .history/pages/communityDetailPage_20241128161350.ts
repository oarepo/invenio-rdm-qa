...
// FIELDS ------------------------------------------------------------------------------  
  ...
  async selectCuratorRole() {
    // Locate the parent div with the role 'Curator' and find the input within it
    const curatorRadioButtonLocator = 'div.ui.radio.checkbox input[aria-label="Curator"]';
  
    // Locate the parent element using the label and ensure it's the correct context
    const parentElementLocator = `div.ui.radio.checkbox input[aria-label="Curator"]:has(+ label:has-text("Curator"))`;
  
    await this.page.locator(parentElementLocator).click();
  }
