...
/**
 * This class abstracts complex UI logic for interacting with a community's detail view,
 * ensuring that automated tests remain readable, maintainable, and DRY.
 *
 * ───────────────────────────────────────────────────────────────────────────────────────
 * KEY RESPONSIBILITIES:
 * ───────────────────────────────────────────────────────────────────────────────────────
 * 1. Navigation:
 *    - Switching between core sections: Records, Members, Requests, Settings, Pages, About, etc.
 *    - Navigating to new or specific subsections (e.g., "Curation policy", "Review policy", "Privileges").
 *
 * 2. Interaction:
 *    - Editing fields: community name, visibility, curation/about pages.
 *    - Managing community membership (invite, remove, role assignment).
 *    - Handling modal dialogs and confirmation steps (e.g., delete, leave).
 *    - Toggling advanced options (e.g., record visibility, version sliders).
 *
 * 3. Verification:
 *    - Asserting visibility or absence of elements (e.g., "Restricted" label, community presence).
 *    - Validating content changes, like name updates or section text.
 *    - Confirming user roles, presence of record management options, and search results.
 *
 * 4. Buttons:
 *    - Consistently wrapped clicking logic for buttons (e.g., Save, Delete, Invite).
 *    - Conditional checks for visibility/enabled state when needed.
 *
 * ───────────────────────────────────────────────────────────────────────────────────────
 * DEPENDENCIES:
 * ───────────────────────────────────────────────────────────────────────────────────────
 * - `UIHelper`: custom helper class for shared navigation and utility methods.
 * - `testData`: static data source for predictable test inputs (e.g., emails, record names).
 * - `urls`: centralized mapping of application routes.
 *
 * ───────────────────────────────────────────────────────────────────────────────────────
 * EXAMPLE USAGE:
 * ───────────────────────────────────────────────────────────────────────────────────────
 * const detail = new CommunityDetail(page);
 * await detail.navigateToSettingsSection();
 * await detail.editCommunityName();
 * await detail.clickSaveButton();
 * await detail.verifyUpdatedCommunityName("My Community_EDITED");
 */
