...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async selectButtonByText(page: Page, buttonText: string, index: number = 0) {
    // Find all matching buttons with dynamic text content
    const buttons = await page.$$(`div.flex button:has-text("${buttonText}")`);

    // Ensure buttons exist before clicking
    if (buttons.length > index) {
      await buttons[index].click(); // Click the button at the specified index
      } else {
      console.error(`Button with text "${buttonText}" at index ${index} not found.`);
      }
    } 
