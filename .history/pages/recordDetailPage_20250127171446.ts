...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async selectCitationStyle(style: string): Promise<void> {
    ...
    const dropdown = this.page.locator('//div[@aria-label="Export selection"]');
  ...
  async selectFileFormat(style: string): Promise<void> {
    ...
    const dropdownIcon = this.page.locator('.ui.selection.dropdown.export-dropdown i.dropdown.icon');
    ...
    const option = menu.locator(`.item span.text:has-text("${style}")`);
    await option.click();
