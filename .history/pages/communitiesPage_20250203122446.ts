...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async selectSortOption(option: string): Promise<void> {
    ...
    const optionLocator = this.page.locator(`div.menu div.item span.text:has-text("${option}")`);
    await optionLocator.first().click();

    // Check how many items match and log them
  const itemsCount = await optionLocator.count();
  console.log(`Found ${itemsCount} items matching option: "${option}"`);

  // If multiple options match, choose the first one (adjust if necessary)
  if (itemsCount > 1) {
    await optionLocator.nth(0).click(); // Targeting the first item
  } else {
    await optionLocator.click(); // If there's only one match, click directly
  }
  }
