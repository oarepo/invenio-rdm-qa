...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async getFirstCommunityName(): Promise<string> {
    ...
    const text = await firstCommunityLocator.textContent();
    if (!text) {
      throw new Error('The first community name is not found or is empty.');
    }
    return text.trim(); // Ensure we return a trimmed string
  }
