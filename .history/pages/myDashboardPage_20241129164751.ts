...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickDeclineButton(index: number): Promise<void> {
    const acceptButton = this.page.locator('button:has-text("Decline")').nth(index);
    await acceptButton.waitFor({ state: 'visible' });
    await acceptButton.click();
  }
  ...
  // Method to click 'Decline' button in 'Decline request' pop up dialog section (accept invitation to the community)
  async clickDeclineButtonConfirm(): Promise<void> {
    const acceptButton = this.page.locator('#accept .actions > button.ui.icon.positive.left.labeled.button:has-text("Decline")');
    await acceptButton.waitFor({ state: 'visible' });
    await acceptButton.click();
  }
