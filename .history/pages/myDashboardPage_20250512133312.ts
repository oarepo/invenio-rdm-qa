...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async isDeclineButtonPresent(index: number): Promise<boolean> {
    return await this.page.locator('button:has-text("Decline")').nth(index).count() > 0;
  }
...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Returns how many "Decline" buttons are currently rendered
   
  async getDeclineButtonsCount(): Promise<number> {
    return await this.page.locator('button:has-text("Decline")').count();
  }
