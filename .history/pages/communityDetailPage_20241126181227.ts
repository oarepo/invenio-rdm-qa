...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to check if the 'Restricted' access status label is present ('Records' section)
  async verifyRestrictedLabel(): Promise<boolean> {
    const restrictedLabelDivLocator = this.page.locator('div.ui.small.horizontal.label.access-status.restricted');
    return await restrictedLabelDivLocator.isVisible();
  }

  // Method to check if the 'Restricted' record is not present (list of record is empty)
  async isNoMatchesHeaderPresent(): Promise<boolean> {
    const noMatchesHeader = this.page.locator('h2.ui.header:has-text("We couldn\'t find any matches for your search")');
    return await noMatchesHeader.isVisible();
  }
}