...
/**
 * This class encapsulates interactions with the record detail page.
 * It provides methods for navigation, UI element interactions (buttons, links),
 * retrieval of record metadata (title, citation), and verification of UI states
 * such as version presence, embargo/restricted labels, and preview availability.
 *
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * INCLUDES:
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * 1. Navigation:
 *    - Navigate to homepage, dashboard, first record detail, and version links.
 *
 * 2. Field retrieval:
 *    - Get record title and citation text.
 *
 * 3. Buttons:
 *    - Click Edit, New version, Share, Export selection, Export, Download, Preview buttons.
 *
 * 4. Selection:
 *    - Select citation style and file export format from dropdowns.
 *
 * 5. Verification:
 *    - Check presence of versions, embargo/restricted labels, access status sections,
 *      restricted messages, preview container and iframe.
 *    - Validate citation format by regex.
 *    - Retrieve selected citation style.
 *
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * EXAMPLE USAGE:
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * const recordDetail = new RecordDetail(page);
 * await recordDetail.navigateToMyDashboard();
 * await recordDetail.firstRecordDetail();
 * const title = await recordDetail.getRecordTitle();
 * console.log(`Record title: ${title}`);
 * await recordDetail.clickEdit();
 * const isV2Present = await recordDetail.isVersionV2Present();
 * console.log(`Version v2 present: ${isV2Present}`);
 */
...
export class RecordDetail {
  ...
  async clickVersionV1() {
    ...
    await this.page
      .locator('div.item a.text-break:has-text("Version v1")')
      .click();
  }
  ...
  async getCitationText(): Promise<string> {
    const citationLocator = this.page.locator(
      '//div[@id="citation-text"]/div[text()]'
    );
  ...
  async clickEdit() {
    const editButton = this.page.locator(
      "#recordManagement > div > div:nth-child(1) > button"
    );
  ...
  async clickNewVersion() {
    const newVersionButton = this.page.locator(
      "#recordManagement > div > div:nth-child(2) > span > button"
    );
  ...
  async clickNewVersion2() {
    const newVersionButton = this.page.getByRole("button", {
      name: "New version",
    });
    await newVersionButton.waitFor({ state: "visible" });
    await newVersionButton.click();
  ...
  async clickShare() {
    const shareButton = this.page.locator(
      '//div[@id="recordManagement"]//button[contains(@class, "ui") and contains(text(), "Share")]'
    );
  ...
  async clickExportSelection() {
    const exportSelectionButton = this.page
      .getByLabel("Export selection")
      .locator("i");
  ...
  async clickDownloadAll() {
    ...
    const downloadAllButton = await this.page.locator(
      '//th/a[@role="button"][contains(@class,"archive-link")]'
    );
  ...
  async clickPreview() {
    const previewButton = await this.page.locator(
      '//span/a[@role="button"][contains(@class,"preview-link")]'
    );
  ...
  async clickDownload() {
    const downloadButton = await this.page.locator(
      '//span/a[@role="button"][not(contains(@class,"preview"))]'
    );
  ...
  async selectCitationStyle(style: string): Promise<void> {
    ...
    await this.page.waitForSelector(
      `.ui.selection.dropdown.citation-dropdown .divider.text:has-text("${style}")`
    );
  }
  ...
  async selectFileFormat(style: string): Promise<void> {
    ...
    const dropdownIcon = this.page.locator(
      '//div[@aria-label="Export selection"]'
    );
    await dropdownIcon.click();

    // Wait for the dropdown menu to be visible
    const menu = this.page.locator(
      '//div[@aria-label="Export selection"][@aria-expanded="true"]'
    );
    await menu.waitFor({ state: "visible" });

    // Locate the desired option and click it
    const option = menu.locator(`.item span.text`, {
      hasText: new RegExp(`^${style}$`),
    });
    await option.click();
  }
  ...
  async isVersionV2Present(): Promise<boolean> {
    const versionItem = this.page.locator(
  ...
  async isVersionV1Present(): Promise<boolean> {
    const versionLabel = this.page.locator("span.label.text-muted", {
      hasText: " | Version v1",
    });
  ...
  async isEmbargoedLabelPresent(): Promise<boolean> {
    const embargoedTag = this.page.locator(
      "span.ui.label.horizontal.small.access-status.embargoed.mb-5"
    );
  ...
  async isEmbargoedStatusSectionPresent(): Promise<boolean> {
    const embargoedStatus = this.page.locator(
      "section#record-access-status.ui.warning.message.rel-mt-1"
    );
  ...
  async isRestrictedLabelPresent(timeout: number = 10000): Promise<boolean> {
    const restrictedTag = this.page.locator(
      "span.ui.label.horizontal.small.access-status.restricted.mb-5"
    );
    ...
    try {
      ...
      if (error instanceof Error) {
        console.error(
          `Restricted status label not found within ${timeout} ms: ${error.message}`
        );
      } else {
        console.error(
          `Restricted status label not found within ${timeout} ms: ${String(
            error
          )}`
        );
      }
  ...
  async isRecordAccessStatusSectionPresent(): Promise<boolean> {
    const accessStatusSection = this.page.locator(
  ...
  async checkRestrictedMessagePresence(
    timeout: number = 10000
  ): Promise<boolean> {
    const restrictedMessage = this.page.locator(
      "div.ui.negative.message.file-box-message"
    );
    ...
    try {
      ...
      if (error instanceof Error) {
        console.error(
          `Restricted message not found within ${timeout} ms: ${error.message}`
        );
      } else {
        console.error(
          `Restricted message not found within ${timeout} ms: ${String(error)}`
        );
      }
      return false;
    }
  ...
  async getSelectedStyle(): Promise<string> {
    return this.page
      .locator(".ui.selection.dropdown.citation-dropdown .divider.text")
      .innerText();
  }
