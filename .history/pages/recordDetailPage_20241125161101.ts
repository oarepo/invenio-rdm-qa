...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async isVersionV2Present(): Promise<boolean> {
    ...
    await versionItemLocator.waitFor({ state: 'visible' });
  ...
  async isVersionV1Present(): Promise<boolean> {
    ...
    await versionLabelLocator.waitFor({ state: 'visible' });
    const isVisible = await versionLabelLocator.isVisible();
    return isVisible;
  }

  // Method to check if the 'Embargoed' status element exists
  async isEmbargoedAccessStatusPresent(): Promise<boolean> {
    const embargoedStatusLocator = this.page.locator('span.ui.label.horizontal.small.access-status.embargoed.mb-5');
    return await embargoedStatusLocator.isVisible();
  }

  // Method to check if the "Embargoed" info message section exists on the page
  async isRecordAccessStatusSectionPresent(): Promise<boolean> {
    const recordAccessStatusLocator = this.page.locator('section#record-access-status.ui.warning.message.rel-mt-1');
    return await recordAccessStatusLocator.isVisible();
  }
}