...
// FIELDS ------------------------------------------------------------------------------  
  ...
  async selectCuratorRole() {
    // Locate the parent div that contains the input and label text 'Curator'
    const curatorRadioButtonLocator = 'div.ui.radio.checkbox:has(input[aria-label="Curator"]:has(+ label:has-text("Curator")))';
  
    // Click the input within that parent div
    await this.page.locator(curatorRadioButtonLocator).locator('input').click();
  }
