...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickCanEditRadioButton(): Promise<void> {
    const radioButtonLocator = this.page.locator('div.ui.radio.checkbox input[type="radio"][value="edit"][aria-label="Can edit"]');
    
    // Ensure the radio button is visible and enabled before clicking
    await radioButtonLocator.waitFor({ state: 'visible' });  // Wait for visibility
  
    // Click the radio button (check it)
