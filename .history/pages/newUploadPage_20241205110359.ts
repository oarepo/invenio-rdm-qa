...
async fillUser(): Promise<void> {
  // Retrieve the email from test data
  const userEmail = testData.existingUserEmail;

  // Refine the locator to specifically target the 'Add people' combobox
  const comboboxLocator = this.page.locator('div[role="combobox"][aria-label="Add people"]');
  const searchIconLocator = comboboxLocator.locator('i.search.icon');
  const inputLocator = comboboxLocator.locator('input.search');

  // Wait until the input field becomes visible
  await inputLocator.waitFor({ state: 'visible' });

  // Fill in the user email into the input field
