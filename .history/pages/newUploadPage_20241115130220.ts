...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickSelectButton() {
    // Find all buttons with the specified aria-label inside div.flex
    const buttons = await this.page.locator('div.flex button[aria-label="Select aspernatur"]');
    
    // Log how many buttons are found
    const buttonCount = await buttons.count();
    console.log(`Number of buttons found: ${buttonCount}`);
    
    if (buttonCount >= 2) {
      // If at least two buttons are found, click the second one
