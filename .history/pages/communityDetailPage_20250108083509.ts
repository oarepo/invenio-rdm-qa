...
// NAVIGATION --------------------------------------------------------------------------
  ...
  async navigateToReviewPolicy(): Promise<void> {
    console.log('Navigating to the Privileges...');
    await this.page.click('#review policy');
  }
...
// FIELDS ------------------------------------------------------------------------------  
  ...
  async fillCurationPolicy(): Promise<void> {
    // Dynamically generate or retrieve the text from testData
    const text = testData.upload.recordDescription(); // Generates random description

    // Locate the iframe that contains the rich text editor
    const iframeLocator = this.page.locator('iframe[class="tox-edit-area__iframe"]').first();
