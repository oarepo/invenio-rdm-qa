...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async selectButton() {
  // Locate the community name link (the <a> tag with the community name) and then find the button in the same parent div
  const communityElement = await this.page.locator('a.ui.small.header.truncate-lines-2');  // This targets the community name link
  
  // Get the community name dynamically (you can also pass it as a parameter if needed)
  const communityName = await communityElement.innerText();

  // Locate the button by finding the parent div of the community name and then clicking the corresponding button
  await this.page.locator(`a[aria-label="${communityName}"] >> ancestor::div//button`).click();
  }
