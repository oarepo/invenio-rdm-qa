...
export class NewCommunity {
  ...
  async navigateToNewCommunities() {
    ...
    await this.page.waitForSelector('h1:has-text("Setup your new community")', {
      state: "visible",
    });
  }
  ...
  async fillCommunityIdentifier() {
    ...
    console.log(
      `Filled the community identifier field with: ${communityIdentifier}`
    );
  }
  ...
  async selectPublicCommunity() {
    await this.page
      .locator("(//input[@type='radio' and @value='public'])[1]")
      .check();
  ...
  async selectRestrictedCommunity() {
    await this.page
      .locator("(//input[@type='radio' and @value='public'])[2]")
      .check();
  ...
  async clickCreateCommunity() {
    await this.page
      .locator("button.ui.icon.positive.left.labeled.button")
      .click();
...
getCommunityName(expectedCommunityName: string) {
  return this.page.locator('div.ui.small.header > a.ui.small.header', { hasText: expectedCommunityName }).first();
}
  ...
  async verifyCommunityName(expectedCommunityName: string) {
    const communityNameLocator = this.getCommunityName(expectedCommunityName);
    await expect(communityNameLocator).toBeVisible();
