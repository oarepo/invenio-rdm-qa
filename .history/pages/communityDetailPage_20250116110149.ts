...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async verifySelectedRoleInDropdown(expectedRole: string): Promise<void> {
    // Locate the active selected item within the dropdown options
    const activeOption = this.page.locator('//div[@role="option" and @aria-selected="true"]//strong');
  
    // Wait for the active option to be visible
    await activeOption.waitFor({ state: 'visible' });
  
    // Get the text content of the active option
    const actualRole = await activeOption.textContent();
  
    // Assert that the active option matches the expected role
    if (actualRole?.trim() !== expectedRole) {
      throw new Error(`Expected role to be "${expectedRole}", but got "${actualRole}"`);
    }
  }
