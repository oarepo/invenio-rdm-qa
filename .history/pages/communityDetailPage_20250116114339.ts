...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async verifyRoleIsManager(index: number = 0): Promise<void> {
    // Wait for the dropdown container with the specified role
    const dropdownContainer = this.page.locator('//div[@aria-label="Role manager"]');
    await dropdownContainer.waitFor({ state: 'attached' });
  
    // Locate all role elements within the dropdown
    const roleElements = dropdownContainer.locator('./div[text()="Manager"]');
    ...
    // Wait for the specific role to be visible and active
