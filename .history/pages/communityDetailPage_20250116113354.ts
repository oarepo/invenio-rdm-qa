...
// VERIFICATION ------------------------------------------------------------------------
  ...
  // Method to verify if the role was successfully set
async verifySelectedRole(expectedRole: string): Promise<void> {
  // Locate the dropdown element with the selected role
  const dropdown = this.page.locator(`//div[@aria-label="Role ${expectedRole.toLowerCase()}"]`);

  // Wait for the dropdown to reflect the expected role
  await dropdown.waitFor({ state: 'visible' });

  // Assert that the dropdown reflects the expected role
  const actualRole = await dropdown.getAttribute('aria-label');
  if (actualRole !== `Role ${expectedRole.toLowerCase()}`) {
    throw new Error(`Expected role to be "${expectedRole}", but got "${actualRole?.replace('Role ', '')}"`);
  }
}
