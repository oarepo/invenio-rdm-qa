...
// FIELDS ------------------------------------------------------------------------------
  ...
  async fillPublicationDateWithToday() {
    ...
    const publicationDateInput = this.page.locator('input[name="metadata.publication_date"]');  // Locate the input field
  
    // Click to focus on the field, wait a moment, then clear and fill
    await publicationDateInput.click({ delay: 100 });
    await publicationDateInput.fill(''); // Clear any existing text
    await publicationDateInput.type(today); // Type the date to ensure each keystroke registers
  }
