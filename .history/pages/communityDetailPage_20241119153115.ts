...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async verifyResultsFound(expectedResultsText: string) {
    const resultsLocator = this.page.locator('div.left.aligned.four.wide.column div.ui.label');
    ...
    const actualText = await resultsLocator.textContent();
    expect(actualText).not.toBeNull();
    await expect(resultsLocator).toHaveText(expectedResultsText);
  
    console.log(`Verified the "results found" element is visible with text: "${expectedResultsText}"`);
  }
}