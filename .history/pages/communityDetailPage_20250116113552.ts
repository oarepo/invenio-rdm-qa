...
// VERIFICATION ------------------------------------------------------------------------
  ...
  async verifyRoleIsManager(): Promise<void> {
    // Locate the element with the specific role and text
    ...
    await roleManagerLocator.waitFor({ state: 'visible' });

    // Assert that the locator is visible
    const isVisible = await roleManagerLocator.isVisible();
    if (!isVisible) {
      throw new Error('The role was not set to "Manager" successfully.');
    }
  }
