 ...
 */
  ...
  async logout() {
    const profileDropdown = this.page.locator('#user-profile-dropdown-btn');
    await profileDropdown.waitFor({ state: 'visible' });
    await profileDropdown.click();
  
    // Zajistí, že dropdown menu je otevřené
    const logoutLink = this.page.locator('a[role="menuitem"]', { hasText: 'Log out' });
