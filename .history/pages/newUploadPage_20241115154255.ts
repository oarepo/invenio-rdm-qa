...
// FIELDS ------------------------------------------------------------------------------
  ...
  async checkFirstMetadataOnlyCheckbox() {
    await this.page.locator('label', { hasText: 'Metadata-only record' }).click();
  }
...
// BUTTONS -----------------------------------------------------------------------------
  ...
  async clickMyCommunitiesTab() {
    await this.page.locator('#my-communities-tab').click();
    console.log('Clicked the "My communities" tab button.');
  }

  // Method to click the 'Select' button ('Select a community' pop-up window)
  async clickSelectButton(communityName: string, index: number) {
    // Locate all community items with the specific community name
    const communityAnchors = this.page.locator(`a:has-text("${communityName}")`);
  
    // Ensure that the index is valid
    const totalItems = await communityAnchors.count();
    if (index < 0 || index >= totalItems) {
      throw new Error(`Index ${index} is out of range. There are only ${totalItems} items.`);
    }
  
    // Get the specific community anchor by index
    const communityAnchor = communityAnchors.nth(index);
  
    // Navigate to the parent container (div with class 'community-item')
    const communityItem = communityAnchor.locator('..').locator('..');
  
    // Find the "Select" button within the specific community item
    const selectButton = communityItem.locator('button.ui.tiny.button');
