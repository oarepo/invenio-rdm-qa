...
export const testData = {
  ...
  randomPassword: () => faker.internet.password(),
   ...
   existingUserEmail: 'petr.brablc@cesnet.cz',
    ...
    // Predefined alternative credentials
  alternativeUsers: {
  ...
  // Method to retrieve credentials by role
  getCredentials: (role: keyof typeof testData.alternativeUsers | symbol) => {
    const roleKey = typeof role === 'symbol' ? String(role) : role;
    const user = testData.alternativeUsers[roleKey as keyof typeof testData.alternativeUsers];
    if (!user) {
  ...
  upload: {
    ...
    resourceType: (currentlySelected: string | null = null): string => {
      ...
      // If a specific type is provided, return it; otherwise, randomly select from the available types
      if (currentlySelected && resourceTypes.includes(currentlySelected)) {
      return currentlySelected; // Return the exact type provided
      }
