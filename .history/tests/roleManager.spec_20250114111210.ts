...
test.describe('Permission and Access Control', () => {
  ...
  test(qase(30, 'Role - Manager'), async () => {  
    ...
    // Select 'Manager' role
    await communityDetail.selectRoleByIndex(3);
    ...
    // Login as a curator
    const newCredentials = testData.getCredentials('manager');
    ...
    // Navigate to the first community -> 'Members' section
    await communityDetail.waitForTwoSeconds();
    await communityDetail.navigateToCommunities();
    await communityDetail.navigateToFirstCommunity();
    await communityDetail.navigateToMembersSection();
