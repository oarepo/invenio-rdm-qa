...
fixtures.test.describe('Communities', () => {
  ...
  fixtures.test.beforeEach(async ({ loggedInPage, createNewCommunity, inviteNewMemberToCommunity }) => {
  ...
  fixtures.test.afterEach(async ({ page }, testInfo) => {
  ...
  fixtures.test(qase(16, 'Accept Invitation'), async () => {  
    ...
    fixtures.expect(isYouLabelVisible).toBe(true);
  });
});