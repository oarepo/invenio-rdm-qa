...
// Check how many #tinymce elements are found
const tinymceElements = await frame.locator('#tinymce').elementHandles();
console.log(`Found ${tinymceElements.length} #tinymce elements.`);

// Choose the first one (if multiple exist)
const tinymceElement = tinymceElements[1]; // or however you determine which one to use

// Interact with the element
await tinymceElement.click();
await tinymceElement.fill('Your text here');
