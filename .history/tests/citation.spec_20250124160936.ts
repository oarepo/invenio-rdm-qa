...
test.describe('Record Landing Page', () => {
  ...
  test(qase(46, 'Citation'), async () => {
    ...
    expect(citationHarvard).toMatch(/^[a-zA-Z]+[\.,] \(\d{4}\)/); // Allow period or comma after author
    expect(citationHarvard).toMatch(/“.*”/); // Title in quotes
    expect(citationHarvard).toMatch(/CERN/); // CERN mentioned
    expect(citationHarvard).toMatch(/doi:\s\d{2}\.\d{5}\/[a-z0-9-]+/); // DOI link
