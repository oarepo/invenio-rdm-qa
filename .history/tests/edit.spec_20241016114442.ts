...
test.describe('Edit record', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {  // CHANGED from afterAll to afterEach
  ...
  test('Edit Existing Record', async () => {  // REMOVED redundant arguments from test function
   ...
   // await uploadPage.page.click('//a[contains(@href, "/records/")][1]');
    ...
    await uploadPage.fillDescription(recordDescription);  // ADDED await

    // Change Resource type with fallback value
    currentlySelectedType = currentlySelectedType || 'defaultType';  // ADDED fallback
    ...
    await uploadPage.fillTitle(recordTitleEdit);  // ADDED await
