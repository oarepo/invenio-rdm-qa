...
// Select the iframe by its title
const frame = page.frameLocator('iframe[title="Rich Text Area"]');

// Now select the element with ID 'tinymce' within that iframe
await frame.locator('#tinymce').click(); // Clicks on the element
await frame.locator('#tinymce').fill('Your text here'); // Fills the contenteditable area
