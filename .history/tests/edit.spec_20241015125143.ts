...
// Locate the iframe based on a stable parent container or nearby element
const frame = page.frameLocator('iframe[title="Rich Text Area"]');

// Then, locate the editable body inside the iframe
await frame.locator('body[contenteditable="true"]').nth(1).click(); // Second element


// Now, type text into the description field
await frame.locator('body[contenteditable="true"]').fill('Your description text here');
