...
test.describe('Record Landing Page', () => {
  ...
  test(qase(46, 'Citation'), async () => {
    ...
    expect(citationHarvard).toMatch(/^[a-zA-Z]+[.,]?\s*\(\d{4}\)/);      // autor + rok
    ...
    expect(citationIeee).toMatch(/\[\d+\]/);                                     // [číslo]
    expect(citationIeee).toMatch(/[a-zA-Z]+,/);                                  // autor,
    expect(citationIeee).toMatch(/CERN/);                                        // CERN
    expect(citationIeee).toMatch(/\d{4}/);                                       // rok
