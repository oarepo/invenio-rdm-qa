...
test.describe('Search and Filter', () => {
  ...
  test(qase(50, 'Communities - Search'), async () => {  
    ...
    // Select the 'Best match' option from the 'Sort by' dropdown
    await uploadPage.waitForTwoSeconds();
    await homePage.selectSortOption("Best match");

    // Validate the 'Sort by' was chosen correctly ('Best match')
    const isBestMatchSelected = await communities.isSortOptionSelected("Best match");
