...
test.describe('Embargo', () => {
  ...
  test(qase(26, 'Full Record Restricted'), async ({ uploadPage }) => {
    ...
    // Verify if the 'Restricted' access status div is present
    await uploadPage.waitForTwoSeconds();
    const restrictedAccessStatusDivVisible = await communityDetail.isRestrictedAccessStatusDivPresent();
    expect(restrictedAccessStatusDivVisible).toBe(true);
  });
});
