...
test.describe('Record Landing Page', () => {
  ...
  test(qase(46, 'Citation'), async () => {
    ...
    const harvardFormatRegex = /^[a-zA-Z0-9\s,]+ \(\d{4}\) „[a-zA-Z0-9\s]+.". [a-zA-Z0-9]+\. doi: [0-9.]+$/;
    ...
    const apaFormatRegex = /^[a-zA-Z0-9\s]+ \(\d{4}\)\. [a-zA-Z0-9\s]+\. [a-zA-Z0-9]+\. https:\/\/doi\.org\/[0-9.]+$/;
    ...
    const ieeeFormatRegex = /^\[\d+\] [a-zA-Z0-9\s]+, „[a-zA-Z0-9\s]+.". [a-zA-Z0-9]+, led. \d{1,2}, \d{4}\. doi: [0-9.]+$/;
