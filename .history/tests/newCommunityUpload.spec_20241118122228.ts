...
test.describe('New Upload', () => {
  ...
  test(qase(8, 'New Community Upload '), async ({ uploadPage }) => {
    ...
    expect(isSubmittedVisible).toBe(true); // Assert that the "Submitted" element is visible
    ...
    expect(isConfirmationVisible).toBe(true); // Assert that the confirmation element is visible
  });
});
