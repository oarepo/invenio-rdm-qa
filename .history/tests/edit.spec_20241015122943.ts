...
// Locate the iframe by attributes other than its id (e.g., title or class)
const frame = page.frameLocator('iframe[class="tox-edit-area__iframe"]');

// Click inside the editable area (the body of the iframe)
await frame.locator('body[contenteditable="true"]:nth-of-type(1)').click();

await frame.locator('html > body[contenteditable="true"]').click();

// Type some text into the rich text editor
await frame.locator('body[contenteditable="true"]').fill('This is a sample description');
