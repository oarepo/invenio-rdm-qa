...
test.describe('Edit record', () => {
  ...
  test.beforeEach(async ({ loggedInPage, uploadFileSuccessfully }) => {
    ...
    // Ensure file upload runs before the test
    await uploadFileSuccessfully();  // Running the fixture to upload the file before the test starts
  });
  ...
  test('Edit Existing Record', async () => {
    // Navigate to the uploaded record's detail page
