 ...
 * can be shared across multiple tests. By organizing initialization, 
 * preconditions, and reusable actions in fixtures, you can create consistent 
 * test environments and simplify individual test cases. Each fixture has lifecycle 
 * hooks (`beforeAll`, `afterAll`, `beforeEach`, `afterEach`), allowing it to manage setup 
...
export const test = base.extend<{
  ...
  uploadMultipleFilesToRecord: (fileCount?: number) => Promise<void>; // New fixture for multiple file uploads
}>({
  ...
  // New fixture for uploading multiple files
  uploadMultipleFilesToRecord: async ({ uploadPage }, use, fileCount = 3) => {
    const currentlySelectedType = "your-type";  // This can be dynamically passed if needed

    // Fill in the form only once (Title, Family Name, DOI, Resource Type)
    await uploadPage.navigateToUploadSection();
    await uploadPage.fillTitle(testData.upload.recordTitle());
    await uploadPage.fillFamilyName(testData.upload.familyName());
    await uploadPage.selectDOIOption(true); // Adjust based on test needs
    ...
    // Upload files multiple times without publishing after each file
    for (let i = 0; i < fileCount; i++) {
      await uploadPage.uploadRandomFile();  // Upload the file
      await uploadPage.waitForTwoSeconds(); // Wait for 2 seconds (if needed)
    }

    // After all file uploads, click publish
    await uploadPage.clickPublish();          // Click the Publish button after all uploads
    await uploadPage.clickPublishOnConfirmation(); // Confirm publishing

    // After all uploads and publishing are done, use the fixture
    await use(); // This allows the test to continue after the fixture has run
  },
});
