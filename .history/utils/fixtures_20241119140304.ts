...
export const test = base.extend<{
  ...
  createCommunityAndUploadFile: async ({ newCommunity, uploadPage }, use) => {
    const createCommunityAndUploadFile = async (currentlySelectedType: string): Promise<void> => {
    
    // Step 1: Create a new community with a dynamic name
    await newCommunity.navigateToNewCommunities();
    const communityName = await newCommunity.fillCommunityName(); // Capture the generated community name
    await newCommunity.fillCommunityIdentifier();
    await newCommunity.clickCreateCommunity();

    // Step 2: Upload a file and associate it with the created community
    await uploadPage.navigateToUploadSection();
    await uploadPage.fillTitle(testData.upload.recordTitle());
    await uploadPage.fillFamilyName(testData.upload.familyName());
    ...
    await uploadPage.clickSelectButton(communityName); // Use the captured community name
  ...
  // Execute the steps with dynamic handling of the community name
  await createCommunityAndUploadFile('your-type'); // Replace 'your-type' as needed
  await use();
  },
});
